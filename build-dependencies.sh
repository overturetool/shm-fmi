...
#echo Remove old build if pressend at $ROOT? "(Y or N)"
#read x
#if [ "$x" == "y" ]; then
#echo Removing old folder
#rm -r $ROOT
#else
#echo Continuing to clone
#fi 
...
CC=x86_64-apple-darwin15-gcc CXX=x86_64-apple-darwin15-g++ CXXFLAGS=-fPIC ./configure --disable-shared --enable-static --host=x86_64-apple-darwin15 --prefix=$PROTOBUF_INSTALL --with-protoc=protoc
	...
	if [[ $ROOT == *"win"* ]]; then
		...
		./configure --host=$COMPILER --build=i686-pc-linux-gnu --with-protoc=protoc --disable-shared --prefix=$PROTOBUF_INSTALL
