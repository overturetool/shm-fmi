...
TEST_F(FMUTest, getTypesPlatform) { fmu.getTypesPlatform(); }

TEST_F(FMUTest, getVersion) { EXPECT_STREQ("2.0", fmu.getVersion()); }

TEST_F(FMUTest, instantiate) { instantiated(); }
...
TEST_F(FMUTest, setDebugLogging) {
  ...
  const char* categories[] = {"all", "debug"};
  fmi2Status status = fmu.setDebugLogging(
      comp, true, 2, categories);  // fmi2Boolean, size_t, const fmi2String[]
...
/*******************************************
 *
 * setup. Note that this state doesn't exists in the 2.0 standard but it should
 *and is reported
 *
 *******************************************/
...
TEST_F(FMUTest, setReal) {
  ...
  fmi2Status status = fmu.setReal(comp, vr, 1, new double[1]{1.1});
...
TEST_F(FMUTest, setInteger) {
  fmi2Component comp = initializing();
  const fmi2ValueReference vr[1] = {intId};
  fmi2Status status = fmu.setInteger(comp, vr, 1, new int[1]{1});
...
TEST_F(FMUTest, setString) {
  fmi2Component comp = initializing();
  const fmi2ValueReference vr[1] = {stringId};
  fmi2Status status = fmu.setString(comp, vr, 1, new fmi2String[1]{"test"});
...
TEST_F(FMUTest, setRealInputDerivatives) {
  ...
  const fmi2ValueReference vr[1] = {stringId};
  fmi2Status status = fmu.setRealInputDerivatives(comp, vr, 1, new int[1]{1},
                                                  new double[1]{1.1});
...
TEST_F(FMUTest, getReal) {
  ...
  const fmi2ValueReference vr[1] = {realIdGet};
...
TEST_F(FMUTest, getBoolean) {
  ...
  const fmi2ValueReference vr[1] = {boolIdGet};
...
TEST_F(FMUTest, getInteger) {
  ...
  const fmi2ValueReference vr[1] = {intIdGet};
...
TEST_F(FMUTest, getString) {
  ...
  const fmi2ValueReference vr[1] = {stringIdGet};
...
TEST_F(FMUTest, getDirectionalDerivative) {
  ...
  fmi2Status status = fmu.getDirectionalDerivative(comp, vr, 1, vr, 1,
                                                   new double[1]{1.1}, values);

  /*
   *    typedef fmi2Status fmi2GetDirectionalDerivativeTYPE(fmi2Component, const
   fmi2ValueReference[], size_t,
